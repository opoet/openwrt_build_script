...
if [ "$platform" = "x86_64" ]; then
    ...
    if [ "$MINIMAL_BUILD" = "y" ]; then
        curl -s $mirror/openwrt/23-config-musl-r8500-minimal > .config
    else
        curl -s $mirror/openwrt/23-config-musl-r8500 > .config
    fi
    ALL_KMODS=y
elif [ "$platform" = "rk3568" ]; then
...
# config-common
if [ "$MINIMAL_BUILD" = "y" ]; then
    [ "$platform" != "bcm53xx" ] && curl -s $mirror/openwrt/23-config-minimal-common >> .config
    echo 'VERSION_TYPE="minimal"' >> package/base-files/files/usr/lib/os-release
else
    [ "$platform" != "bcm53xx" ] && curl -s $mirror/openwrt/23-config-common >> .config
fi
...
# [ "$platform" = "bcm53xx" ] && curl -s $mirror/openwrt/generic/upx_list.txt > upx_list.txt
